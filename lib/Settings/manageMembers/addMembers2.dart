...
class addMembers2 extends StatefulWidget {
  @override
  _AddMembersState createState() => _AddMembersState();
}

class _AddMembersState extends State<addMembers2> {
  ...
  void _saveMember() async {
    ...
    try {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => ManageMembersPage2()),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildAvatarDropdownField() {
    return DropdownButtonFormField<String>(
      ...
      value:
          _selectedAvatar, // Ensure this value matches the "name" field of the Map
      items: _avatars
          .map(
            (avatar) => DropdownMenuItem(
              value: avatar[
                  'name'], // The value here is the "name" field from the Map
              child: Row(
                children: [
                  Image.asset(
                    avatar['image']!,
      ...
      onChanged: (value) => setState(() {
        _selectedAvatar =
            value; // Update the selected avatar based on the "name" field
      }),
